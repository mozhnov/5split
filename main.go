...
type InfoMessage struct {
	...
	TraingType string        // тип тренировки
...
func (r Running) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var run InfoMessage
	run.TraingType = r.TrainingType
	run.Duration = r.Duration
	run.Distance = r.distance()
	run.Speed = r.meanSpeed()
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	meenSpeedMs := w.meanSpeed() * KmHInMsec
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(meenSpeedMs, 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func (w Walking) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var walking InfoMessage
	walking.TraingType = w.TrainingType
	walking.Duration = w.Duration
	walking.Distance = w.distance()
	walking.Speed = w.meanSpeed()
	walking.Calories = w.Calories()
	return walking
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var swimming InfoMessage
	swimming.TraingType = s.TrainingType
	swimming.Duration = s.Duration
	swimming.Distance = s.distance()
	swimming.Speed = s.meanSpeed()
	swimming.Calories = s.Calories()
	return swimming
}
